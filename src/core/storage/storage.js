...
export const GameStorage = {
  ...
  get localStorageKey() {
    return DEV ? "gameTestSave" : "gameSave";
  },

  backupDataKey(saveSlot, backupSlot) {
    return DEV ? `game-backupTestSave-${saveSlot}-${backupSlot}` : `game-backupSave-${saveSlot}-${backupSlot}`;
  },

  backupTimeKey(saveSlot) {
    return DEV ? `game-backupTestTimes-${saveSlot}` : `game-backupTimes-${saveSlot}`;
  },
  ...
  checkPlayerObject(save) {
    ...
    if (save.currency === undefined) return "Save does not have [currency] property";
